...
class RodtapClaimedManagement(Document):
	def validate(self):
		total = 0.0
...
def exp_je_data():
	list_of_je = frappe.db.sql(f"""
		...
		From `tabRodtap Details` as rcm
		Join `tabRodtap Claimed Management` as rd
...
@frappe.whitelist()
def journal_entry_list(start_date=None,end_date=None):
	list_of_je = exp_je_data()
	conditions = ""
	
	if not start_date and not end_date:
		frappe.msgprint("Please add Start Date and End Date First")
