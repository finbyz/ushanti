...
def execute(filters=None):
	...
	if filters.get('supplier'):
		jv_query_filters += " and jv.party = '{}'".format(filters.get('supplier'))
	if filters.get('company'):
		jv_query_filters += " and jv.company = '{}'".format(filters.get('company'))

	if not (filters.get('purchase_invoice') or filters.get('purchase_order')):
		jv_query = frappe.db.sql("""
			...
			where
				...
				and gle.account = twa.account and jv.posting_date between '{}' and '{}' {}
		""".format(filters.get('from_date'), filters.get('to_date'), jv_query_filters),as_dict=1)
...
def get_supplier_map(filters, payment_entries):
	...
	for d in filters["invoices"]:
		if [k for k in supplier_detail
			if k.name == d.get("supplier")]:
