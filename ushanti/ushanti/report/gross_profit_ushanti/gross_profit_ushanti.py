...
def execute(filters=None):
	...
	group_wise_columns = frappe._dict({
		"invoice": ["parent","posting_date", "customer","item_code", \
			...
			"warehouse","batch_no","batch_qty", "gross_profit", "gross_profit_percent","lot_no"],
	...
	def load_invoice_items(self):
		...
		self.si_list = frappe.db.sql("""
			select
				...
				dni.name as dn_detail, dni.parent as delivery_note,dni.qty as batch_qty,
				batch.lot_no
				{sales_person_cols}
			from
				...
				left join `tabBatch` as batch on batch.name=dni.batch_no
				{sales_team_table}
			where
