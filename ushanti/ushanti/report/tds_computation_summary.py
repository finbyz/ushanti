...
def get_result(filters):
	...
	for supplier in filters.supplier:
		...
		try:
			...
			account = []
		total_invoiced_amount, tds_deducted = get_invoice_and_tds_amount(filters,supplier.name, account,
...
def get_invoice_and_tds_amount(filters,supplier, account, company, from_date, to_date, fiscal_year):
	...
	jv_query = frappe.db.sql("""
		select
			jv.name as voucher_no, gle.credit
		from
			`tabJournal Entry` as jv
			JOIN `tabGL Entry` as gle on gle.voucher_no = jv.name
		where
			jv.tds_section_code is not null and jv.pan is not null and jv.party_type = 'Supplier' and jv.posting_date between '{}' and '{}'
			and jv.company = '{}' and jv.party = '{}' and jv.docstatus = 1 and gle.is_cancelled = 0
	""".format(filters.get('from_date'), filters.get('to_date'),filters.get('company'),supplier),as_dict=1)
