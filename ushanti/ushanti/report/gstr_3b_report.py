...
def get_data(self):
	...
	self.gst_details = get_company_gst_details(self)
	...
	self.account_heads = get_account_heads(self)

	get_outward_supply_details(self,"Sales Invoice")
	set_outward_taxable_supplies(self)

	get_outward_supply_details(self,"Purchase Invoice", reverse_charge=True)
	set_supplies_liable_to_reverse_charge(self)

	itc_details = get_itc_details(self)
	set_itc_details(self,itc_details)
	get_itc_reversal_entries(self)
	inward_nil_exempt =get_inward_nil_exempt(self,self.gst_details.get("gst_state"))
	set_inward_nil_exempt(self,inward_nil_exempt)

	self.missing_field_invoices = get_missing_field_invoices(self)
...
def get_itc_details(self):
	...
	itc_igst_acc_amount, itc_cgst_acc_amount, itc_sgst_acc_amount, itc_cess_acc_amount = get_itc_jv_amount(self)
	# # Finbyz Changes End
...
def get_outward_supply_details(self, doctype, reverse_charge=None):
	get_outward_tax_invoices(self,doctype, reverse_charge=reverse_charge)
	get_outward_items(self,doctype)
	get_outward_tax_details(self,doctype)
...
def set_outward_taxable_supplies(self):
	...
	set_inter_state_supply(self,inter_state_supply_details)
...
def get_itc_jv_amount(self):
	...
	for gst_acc in gst_settings_accounts:
		igst_account.append(gst_acc.igst_account)
		cgst_account.append(gst_acc.cgst_account)
		sgst_account.append(gst_acc.sgst_account)
		cess_account.append(gst_acc.cess_account)

	jv_igst_query = frappe.db.sql("""
		select
			"All Other ITC" as eligibility_for_itc, sum(gle.debit) as itc_integrated_tax

		from
			`tabJournal Entry` as jv
			JOIN `tabGL Entry` as gle on gle.voucher_no = jv.name
		where
			jv.party_gstin is not null and jv.party_type = 'Supplier' and
			month(jv.posting_date) = '{}' and year(jv.posting_date) = '{}'
			and jv.company = '{}' and jv.docstatus = 1 and gle.is_cancelled = 0
			and gle.account in %s
	""".format(self.month_no,self.year,self.company),(set(igst_account)),as_dict=1)		

	jv_cgst_query = frappe.db.sql("""
		select
			"All Other ITC" as eligibility_for_itc, sum(gle.debit) as itc_central_tax
		from
			`tabJournal Entry` as jv
			JOIN `tabGL Entry` as gle on gle.voucher_no = jv.name
		where
			...
			and jv.company = '{}' and jv.docstatus = 1 and gle.is_cancelled = 0
			and gle.account in %s
	""".format(self.month_no,self.year,self.company),(set(cgst_account)),as_dict=1)

	jv_sgst_query = frappe.db.sql("""
		select
			"All Other ITC" as eligibility_for_itc, sum(gle.debit) as itc_state_tax
		from
			`tabJournal Entry` as jv
			JOIN `tabGL Entry` as gle on gle.voucher_no = jv.name
		where
			...
			and jv.company = '{}' and jv.docstatus = 1 and gle.is_cancelled = 0
			and gle.account in %s
	""".format(self.month_no,self.year,self.company),(set(sgst_account)),as_dict=1)

	jv_cess_query = frappe.db.sql("""
		select
			"All Other ITC" as eligibility_for_itc, sum(gle.debit) as itc_cess_amount
		from
			`tabJournal Entry` as jv
			JOIN `tabGL Entry` as gle on gle.voucher_no = jv.name
		where
			...
			and jv.company = '{}' and jv.docstatus = 1 and gle.is_cancelled = 0
			and gle.account in %s
	""".format(self.month_no,self.year,self.company),(set(cess_account)),as_dict=1)	
	itc_igst_acc_amount, itc_cgst_acc_amount, itc_sgst_acc_amount, itc_cess_acc_amount = 0, 0, 0, 0

	if jv_igst_query:
		itc_igst_acc_amount += flt(jv_igst_query[0].itc_integrated_tax)

	if jv_cgst_query:
		itc_cgst_acc_amount += flt(jv_cgst_query[0].itc_central_tax)

	if jv_sgst_query:
		itc_sgst_acc_amount += flt(jv_sgst_query[0].itc_state_tax)
	
	if jv_cess_query:
		itc_cess_acc_amount += flt(jv_cess_query[0].itc_cess_amount)
		
	return itc_igst_acc_amount, itc_cgst_acc_amount, itc_sgst_acc_amount, itc_cess_acc_amount
