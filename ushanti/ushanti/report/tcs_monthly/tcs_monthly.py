...
def _execute(filters, additional_table_columns=None, additional_query_columns=None):
	...
	company_tcs_field=frappe.get_value("Company",filters.get("company"),'default_tcs_account')
	...
	for inv in invoice_list:
		...
		for tax_acc in tax_accounts:
			if tax_acc not in income_accounts:
				...
				if (tax_acc != company_tcs_field and company_tcs_field):
					new_total_with_taxes+=tax_amount
				row.update({
		...
		row.update({
			"new_total_with_taxes":( base_net_total or inv.base_net_total) + new_total_with_taxes,
			'tax_total': total_tax,
...
def get_columns(invoice_list, additional_table_columns):
	...
	total_columns = [
		...
		{
			"label": _("Net Total With Taxes"),
			"fieldname": "new_total_with_taxes",
			"fieldtype": "Currency",
			"options": "currency",
			"width": 120,
		},
